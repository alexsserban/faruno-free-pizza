  ...
  <div>
    <form class="w-full">
      <div class="flex flex-wrap mb-6 -mx-3">
        <div class="w-full px-3">
          ...
          <input
            ...
            type="text"
            placeholder="Alexandru Alexandrescu Alex"
            v-model="user.name"
        ...
        <div class="w-full px-3 mb-6 md:w-1/2 md:mb-0">
          ...
          <template v-if="$v.user.phone.$error">
            <p
              ...
              class="text-xs italic text-red-500"
            >
              Completeaza numarul tau de telefon! Vei primi un cod unic cu care
              iti vei ridica comanda.
            </p>
            <p
            ...
            <p v-if="!$v.user.phone.unique" class="text-xs italic text-red-500">
              Acest numar de telefon a fost deja folosit pentru realizarea unei
              comenzi!
            </p>
          </template>
        </div>
        ...
        <div class="w-full px-3 md:w-1/2">
          ...
          <template v-if="$v.user.email.$error">
            ...
            <p v-if="!$v.user.email.unique" class="text-xs italic text-red-500">
              Acesta adresa de email a fost deja folosita pentru realizarea unei
              comenzi!
            </p>
          </template>
        </div>
      ...
      <div class="flex flex-wrap -mx-3s">
        <div class="w-full px-3 mb-6 md:w-1/2 md:mb-0 ">
          ...
          <VueCtkDateTimePicker
            ...
            color="#918085"
            button-color="#5B5750"
            @input="this.$v.user.date.$touch()"
            v-model="user.date"
          >
            <input
              :class="
                $v.user.date.$error ? 'border-red-500' : 'border-gray-200'
              "
              class="w-full px-4 py-3 mt-2 mb-4 leading-tight text-gray-700 bg-gray-200 border border-gray-200 rounded appearance-none focus:outline-none focus:bg-white focus:border-gray-500"
              id="grid-last-name"
              type="text"
              placeholder="05.12.2019 la ora 12:45"
            />
          </VueCtkDateTimePicker>

          <template v-if="$v.user.date.$error">
            <p
              v-if="!$v.user.date.required"
              class="text-xs italic text-center text-red-500"
            >
              Selecteaza Data si Ora la care vrei sa ridici comanda!
            </p>
            <p
              v-if="!$v.user.date.enoughTimeToPrepare"
              class="text-xs italic text-center text-red-500"
            >
              Pentru a-ti oferi cea mai buna pizza, avem nevoie de mai mult timp
              pentru preparare.
            </p>
            <p
              v-if="!$v.user.date.availableSlot"
              class="text-xs italic text-center text-red-500"
            >
        ...
        <div class="w-full px-3 mb-8 md:w-1/2 md:mb-0">
          ...
          <div class="relative mt-2 mb-4">
            <select
              :class="
                $v.user.pizza.$error ? 'border-red-500' : 'border-gray-200'
              "
              v-model="user.pizza"
            ...
            >
              <option value="" selected disabled hidden
                >Alege pizza preferata</option
              >
              <option v-for="(pizza, index) in pizzas" :key="index">{{
                pizza
              }}</option>
            </select>
      ...
      <div class="flex justify-end mt-4 -mx-3s">
        <button
          @click.prevent="send()"
          class="self-end px-4 py-2 font-bold text-white rounded bg-secondary hover:bg-main"
        >
          Trimite
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async send() {
      ...
      // Crate Date Object
      //   const [date, time] = this.user.date.split(" ");
      //   const [year, month, day] = date.split("-");
      //   const [hour, minutes] = time.split(":");
      //   const fullDate = new Date(year, month - 1, day, hour, minutes);
      ...
      swal(
        ...
        `Te asteptam pe data de ${this.user.date} !`,
        "success"
      );
      ...
      //   smsMessage.from = "+40740309402";
      //   smsMessage.to = "+40772232270";

      // smsMessage.body = 'Andreas este cel mai tare!';

      smsMessage.from = "+40726504490";
      ...
      var smsApi = new api.SMSApi(
        "alex@venkon.ro",
        "B6B46C04-8EAE-D901-E3BD-E17A1D2271CB"
      );
